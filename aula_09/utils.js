...
void main(){
   fColor=vec4(1.0, 0.0, 0.0, 1.0);
	...
	PPS(x, y, fillColor = [1, 0, 0, 1], borderColor = [0, 0, 0, 1]) {
        const stack = [];
        const targetColor = this.getPixelColor(x, y);
        const targetColorString = targetColor.toString();

        if (this.colorsMatch(targetColor, fillColor) || !this.colorsMatch(targetColor, borderColor)) {
            return;
        }
        ...
        while (stack.length > 0) {
            ...
            if (this.colorsMatch(this.getPixelColor(currentX, currentY), targetColor)) {
                this.setPixelColor(currentX, currentY, fillColor);
